...
MeleeEnemyBlue::MeleeEnemyBlue(glm::vec2 position, glm::vec2 velocity, Sprite* s, GameState* gs)
...
void MeleeEnemyBlue::update(float elapsed) {
	// cout << time_step << endl;
	time_step += elapsed;
}

void MeleeEnemyBlue::move(float elapsed, const glm::vec2 &player_pos, Map &map) {
...
Bullet* MeleeEnemyBlue::do_attack(const glm::vec2 &player_pos) {
